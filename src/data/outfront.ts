export const outfront = {
  hero: {
    caption: 'OUTFRONT Media, the out-of-home network these four campaigns shipped through.',
  },

  overview: {
    intro: [
      'Over one summer as a Graphic Design Intern at OUTFRONT Media, I designed four out-of-home campaigns for four different clients, each one shipped to real bus kings, liveboards, and a bus wrap across New York City.',
      "Every brief came with the same non-negotiable constraint: a design has to land in the few seconds someone actually looks at it: a glance from a moving bus, a red light at a crosswalk. If it didn't work at a glance, it didn't work.",
    ],
    contribution: [
      "Researched each client's brand voice, website, and existing marketing before opening Photoshop",
      'Designed final layouts, typography, and color direction for every campaign',
      'Explored multiple concepts per brief and presented final direction to the creative team',
      "Followed OUTFRONT's production specs (bleed, visual area, file setup) for print-ready handoff",
    ],
  },

  crossTeam: {
    intro:
      "These four campaigns were my own client work, but they weren't the only creative I touched that summer. I regularly jumped in to support other designers on their briefs, too, from quick concept feedback to full layout passes when a deadline was tight.",
    pieces: [
      { label: 'Team project' },
      { label: 'Team project' },
      { label: 'Team project' },
    ],
  },

  clients: [
    {
      slug: 'ting',
      name: 'Ting Internet',
      brandColor: '#38BDF8',
      header1: { label: 'Project Brief', body: 'Ting Internet delivers high-speed, reliable internet connectivity to communities across the U.S., with a customer-first reputation for fast, friendly service and fiber-optic installs for homes and businesses.' },
      header2: {
        label: 'Design Decisions',
        body: 'I researched Ting\'s site and messaging closely, then carried recurring phrases like "fast, reliable internet" and "reliable fiber internet" straight into the graphics to keep the brand voice consistent. Since the placements were NYC bus kings, I layered in transit-native language, like "All Aboard," "Next Stop," and "Destination," across four colorways to make the ads feel native to the ride, not bolted onto it. The client ran all four simultaneously across the fleet rather than picking one, so every version had to hold up on its own.',
      },
      messaging: {
        hook: 'Make the message feel native to the ride, not bolted onto it.',
        body: "Ting's core message, fast and reliable internet, was already set by the client. My job was finding language that made that message feel like it belonged on a moving bus instead of a landing page, so I wrote four transit-native headline treatments of the same promise.",
        headlines: ['All Aboard: lightning-fast internet', 'Next Stop: unbeatable fast internet', 'Destination: reliable fiber internet', 'Zoom Into: light-speed internet'],
        copyCredit: "Core message provided by the client; the transit-native headline treatment across all four colorways is mine.",
        considerations: ['Brand voice consistency', 'Transit-native language', 'Daily commuter audience', 'Legibility at a glance'],
      },
      timeline: '2 weeks, June 2023',
      tools: ['Adobe Photoshop', 'Adobe Illustrator'],
      deliverables: ['4 bus king colorways', 'NYC transit placement', '4 production templates'],
    },
    {
      slug: 'jack-brackney',
      name: 'The Law Office of Jack Brackney',
      brandColor: '#60A5FA',
      header1: {
        label: 'The Challenge',
        body: "Jack Brackney is a personal injury and employment attorney who helps clients navigate complicated legal situations: disputes with an employer, a creditor, or an injury caused by someone else's negligence. The ad was going on the back of a bus, which meant roughly seven seconds of viewing time at a red light.",
      },
      header2: {
        label: 'My Design Approach',
        body: "I kept the tagline short and impactful, chose imagery tied directly to his legal expertise, and held everything to his firm's style guide. The goal was a design that read instantly, not one that rewarded a second look.",
      },
      messaging: {
        hook: 'Say the one thing a stressed reader needs to hear in seven seconds.',
        body: "This ad runs on the back of a bus a stranger is stuck behind at a red light, often on one of the worst days of their life. There's no room for cleverness. The headline had to answer 'do you handle my situation' before anything else.",
        headlines: ['Need a Personal Injury Advocate? Get to know Jack Brackney.'],
        copyCredit: "Copy followed the firm's existing brand guide and messaging; my job was matching the visual hierarchy and imagery to how that message actually gets read at a red light.",
        considerations: ['Seven-second reading time', 'Message hierarchy', 'Trust signals', 'Brand-guide compliance'],
      },
      timeline: '2 weeks, June 2023',
      tools: ['Adobe Photoshop', 'Adobe Illustrator'],
      deliverables: ['Bus rear king ad', 'Brand-guide-compliant layout'],
    },
    {
      slug: 'its-no-yoke',
      name: "It's No Yoke",
      brandColor: '#FB923C',
      header1: {
        label: 'Background',
        body: "IT'S NO YOKE is a simple, lightweight bag-holding solution that keeps bags from slipping off your shoulders, frees your hands, and helps you stand balanced throughout the day.",
      },
      header2: {
        label: 'Solution',
        body: 'I built the graphic around the product\'s core promise, "balance" and "shoulder-free", and matched the color scheme to the brand\'s black, orange, and white. At the client\'s request, I added a scannable QR code so bus riders could go from a seven-second glance to more product info on their own phone.',
      },
      messaging: {
        hook: 'Every headline had to sell "balance" and "shoulder-free" in five words or less.',
        body: "Rather than pick one headline, I wrote four variations of the same core promise, each leaning on a different word (unburdened, elevate, unleash, ease) so the client could see which language actually landed before committing the ad buy.",
        headlines: [
          'Unburdened Shoulders, From Strain to Strength',
          'Elevate your Load, Set your Shoulders Free',
          'Unleash Hands-Free Ease with Every Step',
          'Where Ease and Balance Travel Hand-in-Hand',
        ],
        copyCredit: 'Headline copy and hashtags (#Relief, #BagBliss) are mine, built around the client\'s core product promise.',
        considerations: ['Product value proposition', 'Wordplay & alliteration', 'Hashtag-ability', 'QR-code call to action'],
      },
      timeline: '2 weeks, July 2023',
      tools: ['Adobe Photoshop', 'Adobe Illustrator'],
      deliverables: ['4 headline variations', 'QR-code tie-in', 'Bus king ad set'],
    },
    {
      slug: 'pike-nurseries',
      name: 'Pike Nurseries',
      brandColor: '#C9A227',
      header1: {
        label: 'The Challenge',
        body: "A collaborative project with graphic interns across regions, developing a shared design theme that reflected Pike Nurseries' values as a landscaping company, spanning a billboard, a liveboard, and a bus wrap.",
      },
      header2: {
        label: 'Process',
        body: "After looking through Pike Nurseries' site and garden designs, I pitched a museum-inspired theme, presenting their landscaping like pieces of art, framed in gold to give the work a gallery feel. The team liked it, and I designed the bus wrap, an animated liveboard, and mobile ads around that concept, then presented the final work to the creative team at OUTFRONT Media.",
      },
      messaging: {
        hook: 'Frame the work like art, then let the copy follow the concept.',
        body: "Once the museum framing was set, the headlines just had to name what the visual was already doing: treating a garden like a gallery piece worth stopping for.",
        headlines: ['Make Your Garden A Masterpiece', 'Make Your Garden A Work Of Art'],
        copyCredit: "Copy developed collaboratively with the regional intern team, built around my museum-inspired concept.",
        considerations: ['Museum/gallery framing', 'Cross-region brand consistency', 'Multi-format adaptation'],
      },
      timeline: '1 month, July to August 2023',
      tools: ['Adobe Photoshop', 'Adobe Illustrator'],
      deliverables: ['Billboard', 'Animated liveboard', 'Bus wrap', 'Mobile ads'],
    },
  ],

  testimonials: [
    {
      client: 'Ting Internet',
      quote: 'A clean, eye-catching design that clearly communicates our fast and reliable internet.',
      initial: 'T',
      color: '#38BDF8',
    },
    {
      client: "It's No Yoke",
      quote: 'A creative design that highlights our product in a simple and engaging way.',
      initial: 'Y',
      color: '#FB923C',
    },
  ],

  takeaways: [
    {
      icon: 'Users',
      title: 'Working with Real Clients',
      body: 'Presenting concepts to account teams and creative directors taught me to explain design decisions in terms of brand goals and audience, not just aesthetics.',
    },
    {
      icon: 'Timer',
      title: 'Designing for Real Constraints',
      body: 'Every brief had to work in the few seconds someone actually looks at a moving bus or a red light. That constraint shaped hierarchy, color, and copy on every layout.',
    },
    {
      icon: 'Network',
      title: 'Cross-Team Collaboration',
      body: "The Pike Nurseries campaign meant syncing weekly with graphic interns across regions, then presenting our final direction to OUTFRONT's creative team.",
    },
    {
      icon: 'TrendingUp',
      title: 'Professional Growth',
      body: 'Seeing my work running on real NYC buses, and hearing directly from clients that it landed, built a kind of confidence a classroom critique never could.',
    },
  ],
}
