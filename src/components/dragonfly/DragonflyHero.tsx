  ...
  return (
    <section className="relative overflow-hidden px-6 pb-10 pt-32 sm:px-10 sm:pb-14 sm:pt-40">
      ...
      <div className="relative mx-auto max-w-6xl">
        ...
        <div className="mt-6 flex flex-col gap-8 lg:flex-row lg:items-center lg:gap-8">
          <div className="flex flex-col gap-3.5 lg:w-[360px] lg:shrink-0">
            <Reveal delay={0.06}>
              <span className="kicker">Case study</span>
            </Reveal>
            <Reveal delay={0.1}>
              <h1 className="text-[clamp(2.1rem,4vw,3.4rem)]">
                Dragonfly <span className="text-gradient">Hot Yoga.</span>
              </h1>
            </Reveal>
            <Reveal delay={0.16}>
              <p className="max-w-lg text-base leading-relaxed text-fg-muted sm:text-lg">{description}</p>
            </Reveal>

            <Reveal delay={0.22}>
              <div className="mt-2 flex flex-wrap divide-x divide-line border-t border-line pt-5">
                {meta.map((item) => (
                  <div key={item.label} className="flex flex-col gap-1 px-4 first:pl-0">
                    <span className="font-mono text-[10px] uppercase tracking-wider text-fg-muted">{item.label}</span>
          ...
          <Reveal delay={0.24} className="relative mx-auto w-full max-w-xs lg:mx-0 lg:max-w-[420px] lg:flex-1">
            <div className="relative aspect-[10/11] w-full overflow-hidden">
              ...
              <motion.img
                ...
                className="absolute inset-0 h-full w-full object-cover drop-shadow-[0_35px_60px_rgba(0,0,0,0.5)]"
              />
            </div>
          </Reveal>
        </div>
